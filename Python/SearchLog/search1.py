...
import json
import pandas as pd
from utils import find_files, find_files_with_list
...
def search_log_aps(files, criteria):
    dict1 = {}
    for file in files:
...
def search_data_aps(file, criteria):
    dict1 = {}
    if file.endswith('.csv'):
        list = find_matching_records(file, criteria)
        if len(list) != 0:
...
def search_log(station_data, path, criteria, station="FTU", global_time_stamp="global_time_stamp"):
    # 打开文件并读取 JSON 数据
    with open('data.json', 'r', encoding='utf-8') as file:
        data = json.load(file)
    ...
    # 遍历列表中的每个对象
    for obj in data:
        key = station + obj["feature"]
        if not key in station_data:
            # 找到数据保存路径
            files = find_files_with_list(path, [station, obj["feature"]], ["LimitLog"], [".csv"], depth=None, match_all_includes=True)
            station_data[key] = merge_csv_files(files)
