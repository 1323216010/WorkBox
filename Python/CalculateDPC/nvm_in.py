...
def dp_correct_in(idraw, nvm , dict1, file_name):
    """
    :param idraw: 原始图像数据
    ...
    for ind_page in range(96):
        for ind_byte in range(64):
            # 将十进制数字转换为十六进制字符串
            nvm_map[ind_byte][ind_page] = format(nvm[ind_byte][ind_page], 'x').upper()

    # 调用 NVM 解码函数
    fd_dp, spd_dp, dp101_dp, dp1001_dp, ZAF_DP, ZAFn_DP, pddc_dp, MPD_DP = nvm_decode_dp_in(nvm_map)
    ...
    # 调用 DP 校正函数
    idraw_comp, rev = dp_comp_rev2p0(idraw, spd_dp, fd_dp, dp101_dp, dp1001_dp, ZAF_DP, ZAFn_DP, pddc_dp, MPD_DP)

    return idraw_comp

def nvm_decode_dp_in(nvm_map):
    """
    解码不同的数据点（DP）。
    ...
    return fd_dp, spd_dp, dp101_dp, dp1001_dp, ZAF_DP, ZAFn_DP, pddc_dp, MPD_DP
...
def get_dp_number(nvm_map):
    ...
    dp_objects = {
        ...
        'MPD_DP': {'addr_bit': 25},
        'SPARE0_DP': {'addr_bit': 25},
        'SPARE1_DP': {'addr_bit': 25}
    }

    offset = 13
    nvm_address_num = 3584  # 初始 NVM 地址数字
    ...
    for dp_name, dp_info in dp_objects.items():
        ...
        # 计算起始和结束地址
        if dp_name == 'FD_DP':
            dp_start_address = nvm_address_num + 11
        else:
...
def NVMDecodeAddr(nvm_map, dp_number, dp_start_address, dp_end_address):
    ...
    """
    dp_addr_offset_x = 8  # 仅用于 PIT
    dp_addr_offset_y = 44  # 仅用于 PIT
...
def NVMDecodeAddrType(NVM_MAP, DP_number, DP_startAddress, DP_endAddress, DP_addr_bitLength_type):
    DP_addr_offset_x = 8  # X轴偏移量
    DP_addr_offset_y = 44  # Y轴偏移量
...
def dp_comp_rev2p0(IDraw,SPD_DP,FD_DP,DP101_DP,DP1001_DP,ZAF_DP,ZAFn_DP,PDDC_DP,MPD_DP):
    rev = '2.032'  # 版本号
    ...
    for indx in range(Couplet_DP_number):
        ...
        x_tmp_1 = DP_address_x_Couplet[(indx - 1) * 2]
        x_tmp_2 = DP_address_x_Couplet[indx * 2]
        y_tmp_1 = DP_address_y_Couplet[(indx - 1) * 2]
        y_tmp_2 = DP_address_y_Couplet[indx * 2]  # 仅适用于EW传感器设置v3翻转
        if type_tmp == 0:
            if 3 <= x_tmp_1 <= w - 4:
    ...
    DP_number_total = sum([d['number'] for d in [FD_DP, DP101_DP, DP1001_DP, ZAF_DP, ZAFn_DP, PDDC_DP, MPD_DP]])
    DP_address_x_total = np.concatenate([d['address_x'] for d in [FD_DP, DP101_DP, DP1001_DP, ZAF_DP, ZAFn_DP, PDDC_DP, MPD_DP]])
    DP_address_y_total = np.concatenate([d['address_y'] for d in [FD_DP, DP101_DP, DP1001_DP, ZAF_DP, ZAFn_DP, PDDC_DP, MPD_DP]])
