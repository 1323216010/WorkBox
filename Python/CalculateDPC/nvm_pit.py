...
def dp_correct_pit(idraw, nvm , dict1, file_name):
    """
    :param idraw: 原始图像数据
    ...
    # 调用 NVM 解码函数
    fd_dp, spd_dp, dp101_dp, dp1001_dp, pddc_dp, pix3_cluster, hl_dp, spare1, longcit_cluster = nvm_decode_dp_pit(nvm_map)
    ...
    with open(dict1['csv_path'] + '\\' + file_name + '_' + 'coordinates.csv', mode='w', newline='') as file:
        ...
        for name, d in [('fd_dp', fd_dp), ('spd_dp', spd_dp), ('dp101_dp', dp101_dp),
                        ('dp1001_dp', dp1001_dp), ('pddc_dp', pddc_dp), ('pix3_cluster', pix3_cluster),
                        ('hl_dp', hl_dp), ('spare1', spare1), ('longcit_cluster', longcit_cluster)]:
    ...
    # 调用 DP 校正函数
    idraw_comp, rev = dp_comp_rev2p03(idraw, fd_dp, spd_dp, dp101_dp, dp1001_dp, pddc_dp, hl_dp, pix3_cluster, spare1, longcit_cluster)

    return idraw_comp

def nvm_decode_dp_pit(nvm_map):
    """
    解码不同的数据点（DP）。
    ...
    pix3_cluster = dp_info['PIX3_DP']
    hl_dp = dp_info['HL_DP']
    spare1 = dp_info['SPARE1_DP']
    longcit_cluster = dp_info['LongCIT_Cluster']
    ...
    pix3_cluster = NVMDecodepix3_cluster(nvm_map, pix3_cluster)
    hl_dp = NVMDecodeHL(nvm_map, hl_dp)
    spare1 = NVMDecodeSpare1(nvm_map, spare1)
    longcit_cluster = NVMDecodeLongCIT_Cluster(nvm_map, longcit_cluster)

    # 旋转各个DP对象
    fd_dp = coordinate_rotated_180(fd_dp)
    spd_dp = coordinate_rotated_180(spd_dp)
    dp101_dp = coordinate_rotated_180(dp101_dp)
    dp1001_dp = coordinate_rotated_180(dp1001_dp)
    pddc_dp = coordinate_rotated_180(pddc_dp)
    pix3_cluster = coordinate_rotated_180(pix3_cluster)
    hl_dp = coordinate_rotated_180(hl_dp)
    spare1 = coordinate_rotated_180(spare1)
    longcit_cluster = coordinate_rotated_180(longcit_cluster)

    return fd_dp, spd_dp, dp101_dp, dp1001_dp, pddc_dp, pix3_cluster, hl_dp, spare1, longcit_cluster

def get_dp_number(nvm_map):
    # 初始化 DP 对象
    dp_objects = {
        'FD_DP': {'addr_bit': 25, 'dp_per_tag': 4},
        ...
        'PIX3_DP': {'addr_bit': 25},
        'HL_DP': {'addr_bit': 25},
        'SPARE1_DP': {'addr_bit': 25},
        'LongCIT_Cluster': {'addr_bit': 25}
    }
    ...
    for dp_name, dp_info in dp_objects.items():
        ...
        if dp_name == 'LongCIT_Cluster':
            ...
            nvm_address_num = nvm_address_num[0]  # 更新 nvm_address_num 为列表中的最后一个元素
        else:
            # 计算 NVM 页面和字节
        ...
        if dp_name == 'FD_DP':
            ...
            dp_start_address = dp_objects[prev_dp]['dp_end_address'] + 1
        # dp_start_address = dp_objects[prev_dp]['dp_end_address'] + 1
        ...
        dp_info.update({
            'number': dp_number,
            'dp_start_address': int(dp_start_address),
            'dp_end_address': int(dp_end_address)
        })

        prev_dp = dp_name  # 记录上一个处理的 DP 名称

        if dp_name != 'SPARE1_DP':
...
def nvm_decode_fd(nvm_map, fd_dp):
    ...
    dp_start_address = fd_dp['dp_start_address']
    dp_end_address = fd_dp['dp_end_address']

    if dp_number > 0:
        dp_address_x_nvm, dp_address_y_nvm = nvm_decode_addr(nvm_map, dp_number, dp_start_address, dp_end_address)
        ...
        for indx in range(dp_number):
            ...
            # FD 2x4 - (0,1)
            dp_address_x[base_index + 1] = dp_address_x_nvm[indx] + 0
            dp_address_y[base_index + 1] = dp_address_y_nvm[indx] + 1
            # FD 2x4 - (0,2)
            dp_address_x[base_index + 2] = dp_address_x_nvm[indx] + 0
            dp_address_y[base_index + 2] = dp_address_y_nvm[indx] + 2
            # FD 2x4 - (0,3)
            dp_address_x[base_index + 3] = dp_address_x_nvm[indx] + 0
            dp_address_y[base_index + 3] = dp_address_y_nvm[indx] + 3
...
def nvm_decode_addr(nvm_map, dp_number, dp_start_address, dp_end_address):
    """
...
def NVMDecodeSPD(NVM_MAP, SPD_DP):
    ...
    DP_startAddress = SPD_DP['dp_start_address']
    DP_endAddress = SPD_DP['dp_end_address']
...
def NVMDecodeDP101(NVM_MAP, DP101_DP):
    ...
    DP_startAddress = DP101_DP['dp_start_address']
    DP_endAddress = DP101_DP['dp_end_address']
...
def NVMDecodeDP1001(NVM_MAP, DP1001_DP):
    ...
    DP_startAddress = DP1001_DP['dp_start_address']
    DP_endAddress = DP1001_DP['dp_end_address']
...
def NVMDecodePDDC(NVM_MAP, PDDC_DP):
    ...
    DP_startAddress = PDDC_DP['dp_start_address']
    DP_endAddress = PDDC_DP['dp_end_address']

    if DP_number > 0:
        DP_Address_X_NVM, DP_Address_Y_NVM = nvm_decode_addr(NVM_MAP, DP_number, DP_startAddress, DP_endAddress)
...
def NVMDecodepix3_cluster(NVM_MAP, pix3_cluster):
    ...
    DP_startAddress = pix3_cluster['dp_start_address']
    DP_endAddress = pix3_cluster['dp_end_address']
...
def NVMDecodeHL(NVM_MAP, HL_DP):
    ...
    DP_startAddress = HL_DP['dp_start_address']
    DP_endAddress = HL_DP['dp_end_address']
...
def NVMDecodeSpare1(NVM_MAP, SPARE1):
    ...
    DP_startAddress = SPARE1['dp_start_address']
    DP_endAddress = SPARE1['dp_end_address']
...
def NVMDecodeLongCIT_Cluster(NVM_MAP, LongCIT_Cluster):
    ...
    DP_startAddress = LongCIT_Cluster['dp_start_address']
    DP_endAddress = LongCIT_Cluster['dp_end_address']
...
def dp_comp_rev2p03(IDraw, FD_DP, SPD_DP, DP101_DP, DP1001_DP, PDDC_DP, HL_DP, pix3_cluster, SPARE1, CIT_DP):
    rev = '2.032'  # 版本号
    ...
    for indx in range(Couplet_DP_number):
        ...
        x_tmp_2 = DP_address_x_Couplet[(indx - 1) * 2]
        x_tmp_1 = DP_address_x_Couplet[indx * 2]
        y_tmp_2 = DP_address_y_Couplet[(indx - 1) * 2]
        y_tmp_1 = DP_address_y_Couplet[indx * 2]  # 仅适用于EW传感器设置v3翻转
        if type_tmp == 0:
            if 3 <= x_tmp_1 <= w - 4:
    ...
    DP_number_total = sum([d['number'] for d in [FD_DP, DP101_DP, DP1001_DP, PDDC_DP, HL_DP, SPARE1]])
    DP_address_x_total = np.concatenate([d['address_x'] for d in [FD_DP, DP101_DP, DP1001_DP, PDDC_DP, HL_DP, SPARE1]])
    DP_address_y_total = np.concatenate([d['address_y'] for d in [FD_DP, DP101_DP, DP1001_DP, PDDC_DP, HL_DP, SPARE1]])
    ...
    # 3px簇补偿
    clusterDP_number_total = pix3_cluster['number']
    clusterDP_address_x_total = pix3_cluster['address_x']
    clusterDP_address_y_total = pix3_cluster['address_y']
